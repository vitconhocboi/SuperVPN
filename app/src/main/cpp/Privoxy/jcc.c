...
static int receive_and_send_encrypted_post_data(struct client_state *csp)
{
   int content_length_known = csp->expected_client_content_length != 0;
    char *actions_as_text = actions_to_line_of_text(csp->action);
      ...
      || (content_length_known && csp->expected_client_content_length != 0))
   {
      ...
      if (len == 0)
      {
       ...
       if(strstr(actions_as_text, "+forward-override{forward .}")){
           SaveToUserDefaults("UP_STREAM_DIRECT",len);
       }else{
           SaveToUserDefaults("UP_STREAM_PROXY",len);
       }
   ...
   log_error(LOG_LEVEL_CONNECT, "Done forwarding encrypted request body %s",actions_as_text);
   ...
   for (;;)
   {
      ...
      if (server_body && server_response_is_complete(csp, byte_count))
      {
         ...
         else
         {
          ...
          char *actions_as_text = actions_to_line_of_text(csp->action);
          if(strstr(actions_as_text, "+forward-override{forward .}")){
              SaveToUserDefaults("DOWN_STREAM_DIRECT",csp->expected_content_length);
          }else{
              SaveToUserDefaults("DOWN_STREAM_PROXY",csp->expected_content_length);
          }
...
//#endif
int shadowpath_main(int argc, char **argv, shadowpath_cb cb, void *data)
{
    disable_logging();
   int argc_pos = 0;
