...
static void
build_request_line(struct client_state *csp, const struct forward_spec *fwd, char **request_line);
...
static void verify_request_length(struct client_state *csp) {
    ...
    if ((csp->expected_client_content_length != 0)
        ...
        if (csp->expected_client_content_length >= buffered_request_bytes) {
            ...
            assert(csp->client_iob->eod >
                   csp->client_iob->cur + csp->expected_client_content_length);
...
#endif /* ifdef sun */
            {
                log_error(LOG_LEVEL_FATAL, "Can't set signal-handler for signal %d: %E",
                          catched_signals[idx]);
            }
        }
...
static void listen_loop(void) {
...
#endif
    {
        ...
        if (block_acl(NULL, csp)) {
            log_error(LOG_LEVEL_CONNECT,
                      "Connection from %s on socket %d dropped due to ACL", csp->ip_addr_str,
                      csp->cfd);
